...
routinebeats = useableflukebeatdeep(find(strcmp(useableflukebeatdeep.MaxOrNormal,'Routine') == 1),:);
routineavgCd = mean(routinebeats.DragCoeffReal);

for a=1:length(useableflukebeatdeep.BeatStart); % Recalculates the Thrust Power For Lunge-Associated tailbeats using mean drag coefficient for routine swimming
    if strcmp(useableflukebeatdeep.MaxOrNormal(a),'Lunge-Associated');
        useableflukebeatdeep.Thrust(a) = (0.5*density*Sa*routineavgCd*((useableflukebeatdeep.AvgSpeeds(a))^3)+(k+1)*mass*((useableflukebeatdeep.BeatEndSpd{a}-useableflukebeatdeep.BeatStartSpd{a})/useableflukebeatdeep.BeatLength{a})*useableflukebeatdeep.AvgSpeeds(a)); % recalculates the thrust power for lunge-associated tailbeat
        useableflukebeatdeep.PercentOfThrust{a} = (useableflukebeatdeep.ThrustDragOffset{a}/useableflukebeatdeep.Thrust(a))*100;
    end
end
