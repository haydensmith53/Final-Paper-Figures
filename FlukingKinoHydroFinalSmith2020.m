...
%   Hayden Smith & William Gough (2020) - updated from previous version
...
for a=1:length(useableflukebeatdeep.BeatStart); % calculate out the coefficient of thrust (Ct) and efficiency using the graphs in Yates
    ...
    useableflukebeatdeep.DragCoeff(a) = ((useableflukebeatdeep.Thrust(a))/(0.5*density*Sa*((useableflukebeatdeep.AvgSpeeds(a))^3))); % calculates the drag coefficient for each tailbeat
...
avgdrag = mean(useableflukebeatdeep.DragCoeff);
